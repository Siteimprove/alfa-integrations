...
test(
  "Puppeteer.toPage() scrapes a page",
  async (t) => {
...
test(
  "Puppeteer.toPage() doesn't change crossorigin attribute when no option is provided",
  async (t) => {
    const browser = await puppeteer.launch({
      headless: true,
      args: ["--no-sandbox", "--disable-dev-shm-usage"],
    ...
    const emptyAttr = await page.evaluate(
      () =>
        (window.document.getElementById("empty") as HTMLLinkElement)
          .crossOrigin,
    );
...
test(
  "Puppeteer.toPage() enforces anonymous crossorigin on links without one, when asked to",
  async (t) => {
    const browser = await puppeteer.launch({
      headless: true,
      args: ["--no-sandbox", "--disable-dev-shm-usage"],
